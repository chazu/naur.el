...
(defvar naur-mode-map
  (let ((map (make-sparse-keymap)))
    (define-key map (kbd "C-c z t") #'naur-toggle-chat)
    (define-key map (kbd "C-c z g") #'naur-goto-code-ref)
    (define-key map (kbd "C-c z r") #'naur-set-code-ref)
    (define-key map (kbd "C-c z c") #'naur-cycle-status)
    (define-key map (kbd "C-c z a") #'naur-agenda)
    (define-key map (kbd "C-c z A") #'naur-archive-conversation)
    (define-key map (kbd "C-c z f") #'naur-fold-toggle-at-point)
    (define-key map (kbd "C-c z s") #'naur-start-agent)
    (define-key map (kbd "C-c z l") #'naur-resume-conversation)
    map)
